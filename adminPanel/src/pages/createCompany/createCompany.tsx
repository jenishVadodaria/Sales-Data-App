...
export const CreateCompany = () => {
  ...
  const [cookies, setCookie, removeCookie] = useCookies(["draftFormData"]);
  ...
  useEffect(() => {
    const draftData = cookies.draftFormData;
    if (draftData) {
      setFormData(draftData);
      const selectedCompanyIds = draftData.peerCompanies;
  ...
  const handleSaveDraft = () => {
    ...
    setCookie("draftFormData", updatedFormData, { path: "/" });
  };
  ...
  const handleChange = (
    ...
    setCookie("draftFormData", { ...formData, [field]: value }, { path: "/" });
  };
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12}>
          <TextField
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              handleChange("revenuePercentage", Number(e.target.value))
            }
            fullWidth
          />
        </Grid>
